...
def visualize_feature_maps(
    ...
    fig.suptitle(
        f"Top {num_channels} Feature Activation Maps - Hierarchy {hierarchy_idx+1} ({H}x{W})",
        fontsize=14,
    )
...
def visualize_top_activating_patches(
    ...
    fig.suptitle(
        f"Top {top_k} Activating Patches - Hierarchy {hierarchy_idx+1}, Channel {channel_idx}",
        fontsize=14,
    )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="results/feature_viz", help="Output directory"
    )
    parser.add_argument("--sample-idx", type=int, default=0, help="Sample index")
    parser.add_argument(
        "--hierarchy", type=int, nargs="+", default=[0, 1, 2], help="Hierarchy levels to visualize"
    )
