        ...
        if time_estimates:
            ...
            recommendations["options"] = {
                "quick_baseline": {
                    ...
                    "epochs": 20,
                    "expected_time": time_estimates.get("20_epochs", {}).get(
                ...
                "full_training": {
                    ...
                    "epochs": 100,
                    "expected_time": time_estimates.get("100_epochs", {}).get(
                        "time_formatted", "unknown"
                    ),
...
def plot_training_curves(metrics: Dict, output_dir: Path):
    ...
    axes[2].axhline(
        y=np.mean(speeds), color="red", linestyle="--", label=f"Mean: {np.mean(speeds):.2f} it/s"
    )
...
def generate_report(analysis: Dict, output_path: Path):
    ...
    with open(output_path, "w") as f:
        ...
        if speed.get("status") == "analyzed":
            ...
            f.write(
                f"- **Speed Range**: {speed['min_iterations_per_sec']:.2f} - {speed['max_iterations_per_sec']:.2f} it/s\n"
            )
...
def main():
    ...
    parser.add_argument(
        "--log", type=Path, default=Path("training_run.log"), help="Path to training log file"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    analysis = {
        "config": data["config"],
        "loss_analysis": analyze_loss_curve(data["metrics"]["losses"], data["metrics"]["steps"]),
        "speed_analysis": analyze_training_speed(
            data["metrics"]["iterations_per_sec"], data["metrics"]["steps"]
        ),
    }
