...
def load_model(checkpoint_path: str, device: str) -> Tuple:
    ...
    model = create_hjepa(
        ...
        use_flash_attention=config.get("model", {}).get(
            "use_flash_attention", False
        ),  # Need explicit attention for visualization
    )
...
def visualize_attention_rollout(
    ...
    rollout_resized = np.array(Image.fromarray(rollout_grid).resize((224, 224), Image.BILINEAR))
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="results/attention_rollout", help="Output directory"
    )
    parser.add_argument(
        "--sample-idx", type=int, nargs="+", default=[0, 10, 20], help="Sample indices"
    )
    parser.add_argument(
        "--discard-ratio", type=float, default=0.1, help="Ratio of low attentions to discard"
    )
    ...
    for sample_idx in args.sample_idx:
        ...
        visualize_attention_rollout(
            sample_image, rollout, save_path=output_dir / f"rollout_sample_{sample_idx}.png"
        )
