...
from models.encoder import RoPEAttentionWrapper, VisionRoPE2D, create_encoder
...
def test_encoder_with_rope():
    ...
    assert isinstance(
        first_block_attn, RoPEAttentionWrapper
...
def test_gradient_flow():
    ...
    assert (
        context_encoder.vit.blocks[0].attn.attn.qkv.weight.grad is not None
    ...
    print(
        f"  - QKV weight grad norm: {context_encoder.vit.blocks[0].attn.attn.qkv.weight.grad.norm():.6f}"
    )
